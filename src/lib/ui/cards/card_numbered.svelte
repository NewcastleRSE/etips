...
{#if card}
	<div
		...
		class="card-numbered relative grid grid-cols-2 grid-rows-1"
		class:nested
	>
		<div class="card-numbered-left-col flex">
			<div class="card-numbered-number ml-4 mt-4 flex-shrink-0 p-2 text-2xl">{card.number}</div>
		...
		{#if card.media && card.media?.length > 0}
			<div class="card-numbered-right-col">
				{#each card.media as m}
					{#if m.directus_files_id && typeof m.directus_files_id !== 'string'}
						<Picture image={m.directus_files_id}></Picture>
					{/if}
				{/each}
			</div>
		{/if}
